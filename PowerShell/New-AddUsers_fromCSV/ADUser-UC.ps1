...
# если пользователь не существует
  ...
  {
    New-ADUser -PassThru -Path $NewUserOU -DisplayName $DisplgetayName -Name $DisplayName -GivenName $givenName -SamAccountName $sAMAccountName -Surname $surName -UserPrincipalName $userPrincipalName -Enabled $True -ChangePasswordAtLogon $False -PasswordNeverExpires $True -AccountPassword (ConvertTo-SecureString $userPassword -AsPlainText -Force) -CannotChangePassword $True -City $City -Company $Company -Department $Department -Description "$Description" -OtherAttributes @{'extensionAttribute1'="online"; 'extensionAttribute2'="uuc"}
    ...
    Set-RemoteMailbox -Identity $sAMAccountName -PrimarySmtpAddress $sAMAccountName$Domain
